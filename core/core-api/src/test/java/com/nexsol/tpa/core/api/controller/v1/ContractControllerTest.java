...
public class ContractControllerTest extends RestDocsTest {
	...
	private ExcelGeneratorFactory excelGeneratorFactory;
	...
	public void setUp(RestDocumentationContextProvider restDocumentation) {
		...
		excelGeneratorFactory = mock(ExcelGeneratorFactory.class);
		...
		mockMvc = MockMvcBuilders.standaloneSetup(new ContractController(contractService, excelGeneratorFactory))
	...
	@Test
	@DisplayName("계약 목록 엑셀 다운로드 API 문서화")
	void downloadExcel() throws Exception {
		// Given
		ContractExcelGenerator mockGenerator = mock(ContractExcelGenerator.class);

		// 1. 서비스가 데이터를 반환하도록 설정 (빈 리스트라도 상관없음)
		given(contractService.downloadContracts(any())).willReturn(List.of());

		// 2. 팩토리가 생성기를 반환하도록 설정
		given(excelGeneratorFactory.getGenerator(anyLong())).willReturn(mockGenerator);

		// When & Then
		mockMvc
			.perform(get("/v1/admin/travel/contract/excel").param("startDate", "2025-01-01")
				.param("endDate", "2025-01-31")
				.param("partnerName", "TPA KOREA")
				.param("targetInsurerId", "1") // 엑셀 양식 선택 파라미터
				.contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isOk())
			.andDo(document("contract-excel-download", queryParameters(
					// 검색 조건 (ContractSearchRequest)
					parameterWithName("startDate").description("조회 시작일 (yyyy-MM-dd) *필수"),
					parameterWithName("endDate").description("조회 종료일 (yyyy-MM-dd) *필수"),
					...
					// 엑셀 전용 파라미터
					parameterWithName("targetInsurerId").description("엑셀 양식 보험사 ID (미입력 시 기본 양식)").optional())));
	}
