...
public class PlanControllerTest extends RestDocsTest {
    ...
    void getPlans() throws Exception {
        // Given
        List<Plan> mockPlans = List.of(
                new Plan(1L, "PLAN001", "알뜰 플랜", "해외여행자보험 알뜰 플랜", 1L),
                new Plan(2L, "PLAN002", "표준 플랜", "해외여행자보험 표준 플랜", 1L),
                new Plan(3L, "PLAN003", "프리미엄 플랜", "해외여행자보험 프리미엄 플랜", 1L)
        );

        given(planService.getActivePlans()).willReturn(mockPlans);

        // When & Then
        mockMvc.perform(get("/v1/admin/travel/plan")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(document("plan-list",
                        queryParameters(
                                parameterWithName("insurerId").description("보험사 ID (필터링용, 선택)").optional()
                        ),
                        responseFields(
                                fieldWithPath("result").description("API 실행 결과 (SUCCESS/ERROR)"),
                                fieldWithPath("error").description("에러 정보 (성공 시 null)").optional(),
                                fieldWithPath("data").description("플랜 목록"),
                                fieldWithPath("data[].id").description("플랜 ID"),
                                fieldWithPath("data[].code").description("플랜 코드"),
                                fieldWithPath("data[].name").description("플랜명"),
    ...
    void getPlansByInsurerId() throws Exception {
        ...
        Long insurerId = 1L;
        List<Plan> mockPlans = List.of(
                new Plan(1L, "PLAN001", "알뜰 플랜", "해외여행자보험 알뜰 플랜", insurerId),
                new Plan(2L, "PLAN002", "표준 플랜", "해외여행자보험 표준 플랜", insurerId)
        );
        ...
        mockMvc.perform(get("/v1/admin/travel/plan")
                ...
                .andDo(document("plan-list-by-insurer",
                        queryParameters(
                                parameterWithName("insurerId").description("보험사 ID (필터링용)")
                        ),
                        responseFields(
                                fieldWithPath("result").description("API 실행 결과 (SUCCESS/ERROR)"),
                                fieldWithPath("error").description("에러 정보 (성공 시 null)").optional(),
