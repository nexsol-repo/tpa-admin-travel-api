package com.nexsol.tpa.client.meritz.city;
...
public class MeritzCityProvider implements CityProvider {
	...
	public List<CityInfo> searchCities(String keyword, String type) {
		...
		if (!res.success()) {
			throw new CoreException(CoreErrorType.REFERENCE_API_FAILED,
	...
	private List<CityInfo> parseCityList(Object data) {
		try {
			...
			for (JsonNode item : citiesNode) {
				result.add(new CityInfo(item.path("cityNatlCd").asText(null), item.path("korNatlNm").asText(null),
						item.path("engNatlNm").asText(null), item.path("korCityNm").asText(null),
						item.path("engCityNm").asText(null), item.path("trvRskGrdeCd").asText(null), null));
			}
			return result;
		}
		catch (Exception e) {
